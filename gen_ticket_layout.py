def generate_code(data):
    ...
    if WrittenCode["active"] == "True":
        ...
        draw.text(
            (WrittenCode["posX"], WrittenCode["posY"]),
            str(data["code"]),
            font=font,
            fill=WrittenCode["color"],
        )
        # save image
    ...
    if Qrcode["active"] == "True":
        ...
        qr_code_image = qrcode.QRCode(
            box_size=10, border=0, error_correction=qrcode.constants.ERROR_CORRECT_L
        )
    ...
    if EventDate["active"] == "True":
        ...
        draw.text(
            (EventDate["posX"], EventDate["posY"]),
            EventDate["date"],
            font=font,
            fill=EventDate["color"],
        )
    if debug["active"] == "True":
        img.show()
    ...
    if CreationDate["active"] == "True":
        ...
        font = ImageFont.truetype(
            "font.ttf", CreationDate["fontSize"]
        )  # get current date in format
        from datetime import datetime

        date = data["creation_date"].strftime(CreationDate["format"])
        draw.text(
            (CreationDate["posX"], CreationDate["posY"]),
            date,
            font=font,
            fill=CreationDate["color"],
        )
    if debug["active"] == "True":
        img.show()
    import os
